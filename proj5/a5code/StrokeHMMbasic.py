...
class HMM:
	...
	for y in self.states:
		V[0][y] = self.priors[y] * self.getEmissionProb(y,data[0])
        	path[y] = [y]
   	...
   	for t in range(1, len(data)):
        	...
        	for y in self.states:
				...
				#print self.transitions[y0][y]
				#print self.getEmissionProb(y, data[t])
			(prob, state) = max((V[t-1][y0] * self.transitions[y0][y] * self.getEmissionProb(y, data[t]), y0) for y0 in self.states)
                	V[t][y] = prob
...
class test:
	...
	self.hmm.transitions = {'sunny': {'sunny':0.50, 'cloudy':0.375,'rainy':0.125}, 'cloudy':{'sunny':0.250, 'cloudy':0.125, 'rainy':0.625}, 'rainy':{'sunny':0.250, 'cloudy':0.375, 'rainy':0.375}}
...
class StrokeLabeler:
	...
	for label in self.labels:
		confusion[label] = {}
		for another_label in self.labels:
			confusion[label][another_label] = 0
	for i in range(0, len(classifications)):
		for j in range(0, len(classifications[i])):
			confusion[trueLabels[i][j]][classifications[i][j]] += 1
	print confusion
	return confusion

    def featureTest(self):
    	trainingDir = "../trainingFiles"
	for fFileObj in os.walk(trainingDir):
        ...
        trainingFiles = [ trainingDir + "/" + f for f in goodList ] 
        testFiles = ['../trainingFiles/0128_1.6.1.labeled.xml', '../trainingFiles/0128_1.7.1.labeled.xml', '../trainingFiles/0128_1.8.1.labeled.xml', '../trainingFiles/0128_3.5.1.labeled.xml','../trainingFiles/0128_3.6.1.labeled.xml', '../trainingFiles/0128_6.1.1.labeled.xml', '../trainingFiles/0268_1.4.1.labeled.xml', '../trainingFiles/0268_1.6.1.labeled.xml', '../trainingFiles/0268_1.8.1.labeled.xml', '../trainingFiles/0268_1.9.1.labeled.xml']
	self.hmm = HMM( self.labels, self.featureNames, self.contOrDisc, self.numFVals )
	...
	classifications = []
        for f in trainingFiles:
            print "Loading file", f, "for confusion matrix"
        ...
        self.hmm.train(allObservations, trueLabels)
	for f in testFiles:
	    strokes, labels = self.loadLabeledFile(f)
            classifications.append( self.labelStrokes(strokes))
	self.confusion(trueLabels, classifications)
